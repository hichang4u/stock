...
- F1은 8~10% 고갭 후보를 정적 VI 근접만으로 제거하지 않는다. 예상 체결대금 10억원 이상인 경우에만 허용하며, 실제 VI 활성 여부는 F3 주문 직전에 확인한다.
...
- **08:00 이후에는 PC를 재시작하지 않는다.** 워치독은 1분 주기이고 부팅까지 더하면 2~3분간 프로세스가 없다. 개장 직전이면 09:00 진입을 통째로 놓친다 (2026-09-17 08:54 수동 재시작 → 08:56 워치독 복구, 4분 차이로 진입은 살았다). 재시작이 필요하면 전날 장 마감 후나 08:00 전에 끝낸다.
