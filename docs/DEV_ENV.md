...
──────────────────────────────────────────────────
  ...
  5. 판단을 data/logs/watchdog_<YYYYMMDD>.log 에 남긴다.
     작업 스케줄러는 stdout 을 버리므로 실행 이력에는 "결과 0"만 남고
     무엇을 보고 그렇게 판정했는지는 알 수 없다. 1분 간격이라 하루 종일
...
● 장중 PC 재시작 금지 (08:00~15:30):
  워치독(Task Scheduler, 1분 주기)이 죽은 프로세스를 다시 띄우지만
  부팅 + 감지 지연으로 2~3분간 프로세스가 없다. 이 공백이 08:59~09:01에
  걸리면 그날 진입을 통째로 놓친다.
  사례: 2026-09-17 08:54:34 시작 메뉴에서 수동 재시작(System 이벤트 1074)
  → 08:55:21 부팅 → 08:56:03 워치독 복구 → 09:00 진입 정상. 4분 차이였다.
  main.py 로그에는 재시작 원인이 남지 않는다 — 원인 불명 사망으로 보이면
  System 이벤트 로그의 1074/6006/6005를 먼저 확인한다.
...
F1_HIGH_GAP_MIN_EXPECTED_AMOUNT=1000000000
